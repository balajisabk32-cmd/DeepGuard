"""Capsule-Forensics-v2 network — VENDORED, not authored here.

Source : Nguyen et al., "Use of a Capsule Network to Detect Fake Images and
         Videos" — via FakeAVCeleb/Unimodal/Capsule-Forensics-v2/model_big.py
Licence: see REFERENCE/THIRD_PARTY_LICENCES.md.

The checkpoint contains ONLY the capsule head. VggExtractor is built from
torchvision's ImageNet vgg19, so those weights are part of the trained pipeline
and must be present for the model to mean anything.
"""
